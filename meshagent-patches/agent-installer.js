...
// capturedrv.sys is downloaded from our GitHub release and cached in the agent folder
var CAPTURE_DRIVER_URL     = 'https://github.com/HackMe7822/Mesh-Central/releases/latest/download/capturedrv.sys';
...
function installCaptureDriver()
{
    ...
    try
    {
        ...
        // Download capturedrv.sys from GitHub if not already present
        if (!fs.existsSync(CAPTURE_DRIVER_PATH))
        {
            // Use PowerShell to download (avoids needing a full http client)
            var dl = child_process.execFile(
                process.env['windir'] + '\\System32\\WindowsPowerShell\\v1.0\\powershell.exe',
                ['-ExecutionPolicy', 'Bypass', '-Command',
                 'Invoke-WebRequest -Uri "' + CAPTURE_DRIVER_URL + '" -OutFile "' + CAPTURE_DRIVER_PATH + '" -UseBasicParsing']
            );
            dl.waitExit();
        }
        ...
        if (!fs.existsSync(CAPTURE_DRIVER_PATH))
        {
            process.stdout.write(' [SKIP - could not download driver]\n');
            return;
        }

        // Enable test signing (required until EV cert is obtained)
        ...
        var sc_create = child_process.execFile(
        ...
        var sc_start = child_process.execFile(
            process.env['windir'] + '\\System32\\sc.exe',
        ...
        process.stdout.write(' [DONE]\n');
        process.stdout.write('   -> NOTE: Reboot may be required for test signing to take effect.\n');
    }
    catch(e)
    {
